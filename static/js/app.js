...
<div style="text-align:center;color:#6a7a8e;font-size:12px;margin-top:10px;">Дата последнего обновления: 19.06.2026</div>
`;

// ============ ИНИЦИАЛИЗАЦИЯ ============
document.addEventListener('DOMContentLoaded', function() {
    // Загружаем текст соглашения
    document.getElementById('termsContent').innerHTML = TERMS_TEXT;
...
function checkSubscription() {
    ...
    showModal('📺 ПРОВЕРКА ПОДПИСКИ', `
        <div style="text-align:center;padding:10px 0;">
            <div style="font-size:16px;color:#00d4ff;padding:8px 0;">
                1. Подпишись на канал: <strong>@ARTCSSKINS</strong>
            </div>
            <button class="case-btn" onclick="window.open('https://t.me/ARTCSSKINS', '_blank')" style="margin:10px 0;">
                📺 ПЕРЕЙТИ В КАНАЛ
            </button>
            <div style="font-size:14px;color:#6a7a8e;padding:8px 0;">
                2. После подписки нажми кнопку проверки
            </div>
            <button class="case-btn primary" onclick="verifySubscription()" style="margin:10px 0;">
                ✅ ПРОВЕРИТЬ ПОДПИСКУ
            </button>
...
function verifySubscription() {
    ...
    document.getElementById('modalBody').innerHTML = `
        <div style="text-align:center;padding:20px;">
            ...
            <div style="color:#6a7a8e;padding:10px 0;">Проверяем подписку...</div>
        </div>
    `;
    ...
    .catch(() => {
        showModal('❌ Ошибка', 'Не удалось проверить подписку. Попробуй позже.');
    });
}
    ...
    .then(data => {
        ...
        showModal('📤 ВЫВОД', `
            <div style="text-align:center;padding:10px 0;">
                ...
                <div style="color:#ff4444;font-size:16px;font-weight:700;padding:12px 0;background:rgba(255,0,0,0.1);border-radius:8px;border:1px solid rgba(255,0,0,0.2);">
                    ⚠️ ДЛЯ ВЫВОДА ОБЯЗАТЕЛЬНО НАПИШИ В ПОДДЕРЖКУ СО СКРИНОМ ДАННОГО ОКНА
                </div>
        ...
        if (data.success) {
            ...
            showModal('✅ ЗАЯВКА ОТПРАВЛЕНА!', `
                <div style="text-align:center;">
                    ...
                    <div style="color:#ff4444;font-size:14px;font-weight:700;padding:8px 0;">
                        📌 ДЛЯ ВЫВОДА ОБЯЗАТЕЛЬНО НАПИШИ В ПОДДЕРЖКУ СО СКРИНОМ ЭТОГО ОКНА
                    </div>
                    <div style="color:#6a7a8e;font-size:14px;padding:8px 0;">
                        Предмет: ${name} (${price} 🪙)
                    </div>
...
// ============ АДМИН-ПАНЕЛЬ (65 ФУНКЦИЙ, В МОДАЛЬНОМ ОКНЕ) ============
function loadAdminPanel() {
    ...
    content.innerHTML = `
        <div style="display:flex;flex-direction:column;gap:6px;padding-bottom:20px;">
            ...
            <button class="case-btn" onclick="adminAddSkinToCase()">➕ Добавить скин в кейс</button>
            <button class="case-btn" onclick="adminRemoveSkinFromCase()">➖ Удалить скин из кейса</button>
    ...
    .then(data => {
        ...
        if (data.users && data.users.length > 0) {
            data.users.forEach(u => {
                ...
                html += `<div class="inventory-item"><span>${u.username} ${frozen} ${banned}</span><span>${u.coins} 🪙</span><span>⭐ Lv.${u.level}</span></div>`;
            });
        } else {
...
function adminAddSkinToCase() {
    showModal('ℹ️ В РАЗРАБОТКЕ', 'Функция добавления скинов в кейс будет доступна позже');
}

function adminRemoveSkinFromCase() {
    showModal('ℹ️ В РАЗРАБОТКЕ', 'Функция удаления скинов из кейса будет доступна позже');
}
...
function adminCreatePromo() {
    ...
    fetch('/api/admin/create_promo', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({code: code, reward: parseInt(reward), uses: parseInt(uses)})
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            document.getElementById('adminInfo').textContent = `✅ Промокод создан: ${code} (награда: ${reward} 🪙, использований: ${uses})`;
        } else {
            document.getElementById('adminInfo').textContent = '❌ Ошибка: возможно, такой код уже существует';
        }
    });
}
...
function adminCreatePersonalPromo() {
    showAdminConfirm('Введите ID игрока', 'Введите награду (монеты)', (uid, reward) => {
        ...
        fetch('/api/admin/create_personal_promo', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({user_id: parseInt(uid), reward: parseInt(reward), code: code})
        })
        .then(res => res.json())
        .then(data => {
            document.getElementById('adminInfo').textContent = data.success ? `✅ Личный промокод: ${code} для игрока ${uid}` : '❌ Ошибка';
        });
    });
}
    ...
    .then(data => {
        ...
        if (data.promos && data.promos.length > 0) {
            data.promos.slice(0, 20).forEach(p => {
                const target = p.target ? `(для ${p.target})` : '';
                html += `<div class="inventory-item"><span>${p.code}</span><span>${p.reward} 🪙</span><span>осталось: ${p.uses_left}</span><span style="font-size:11px;color:#6a7a8e;">${target}</span></div>`;
            });
        } else {
...
function adminDeleteUser() {
    showAdminConfirm('Введите ID игрока', '', (uid) => {
        ...
        .then(res => res.json())
        .then(data => {
            document.getElementById('adminInfo').textContent = data.success ? '✅ Пользователь удалён!' : '❌ Ошибка';
        });
    });
}
...
function adminToggleQuiz() {
    ...
    .then(res => res.json())
    .then(data => {
        document.getElementById('adminInfo').textContent = data.success ? `✅ Викторина ${data.enabled ? 'включена' : 'выключена'}` : '❌ Ошибка';
    });
}
...
function adminToggleReferrals() {
    ...
    .then(res => res.json())
    .then(data => {
        document.getElementById('adminInfo').textContent = data.success ? `✅ Рефералы ${data.enabled ? 'включены' : 'выключены'}` : '❌ Ошибка';
    });
}
...
function adminTopCoins() {
    ...
    .then(data => {
        ...
        data.users.forEach((u, i) => {
            ...
            html += `<div class="inventory-item"><span>${medal} ${u.username}</span><span>${u.coins} 🪙</span></div>`;
        });
        document.getElementById('adminInfo').innerHTML = html;
    });
}
...
function adminTopLevel() {
    ...
    .then(data => {
        ...
        data.users.forEach((u, i) => {
            const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i+1}.`;
            html += `<div class="inventory-item"><span>${medal} ${u.username}</span><span>⭐ ${u.level}</span></div>`;
        });
        document.getElementById('adminInfo').innerHTML = html;
    });
}
...
function showAdminConfirm(idPrompt, valuePrompt, callback) {
    ...
    .then(data => {
        ...
        const frozen = data.is_frozen ? '❄️ Заморожен' : '✅ Активен';
        const banned = data.is_banned ? '🚫 Забанен' : '✅ Не забанен';
